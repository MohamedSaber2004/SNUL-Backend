...
using SNUL.Gateway.API.Options;
using SNUL.Shared;
...
namespace SNUL.Gateway.API
{
    public class Program
    {
        public static async Task Main(string[] args)
        {
            ...
            Log.Information("SNUL API Gateway is starting up at {Time}", DateTime.Now);
            ...
            builder.Services.AddSnulSharedDependencies();
            ...
            builder.Services.AddSingleton<SNUL.Gateway.API.Services.OpenApiAggregatorService>();
            builder.Services.AddHostedService<SNUL.Gateway.API.Services.OpenApiCacheWarmer>();
...
#pragma warning disable ASP0014
            ...
            {
                endpoints.MapGet("/openapi/all.json", async (HttpContext httpContext, SNUL.Gateway.API.Services.OpenApiAggregatorService aggregator, CancellationToken ct) =>
                {
                ...
                {
                    options.WithTitle("SNUL Microservices Platform API")
                ...
                foreach (var doc in microserviceDocRoutes)
                {
                    ...
                    endpoints.MapGet(doc.DocRoute, async (HttpContext httpContext, SNUL.Gateway.API.Services.OpenApiAggregatorService aggregator, CancellationToken ct) =>
                    {
                        var gatewayBaseUrl = $"{httpContext.Request.Scheme}://{httpContext.Request.Host}";
