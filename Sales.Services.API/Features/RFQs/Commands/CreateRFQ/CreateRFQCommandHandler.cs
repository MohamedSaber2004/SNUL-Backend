...
using RFQItemEntity = SNUL.Shared.Domain.Models.RFQItem;
namespace Sales.Services.API.Features.RFQs.Commands.CreateRFQ
{
    public class CreateRFQCommandHandler : IRequestHandler<CreateRFQCommand, Result<RFQDto>>
    {
        ...
        {
            var companyRepo = _uow.GetRepository<SNUL.Shared.Domain.Models.Company, Guid>();
            ...
            var productRepo = _uow.GetRepository<SNUL.Shared.Domain.Models.Product, Guid>();
            ...
            var rfq = new RFQEntity { Id = Guid.NewGuid(), RFQNumber = $"RFQ-{DateTime.UtcNow:yyyyMMdd}-{Guid.NewGuid().ToString()[..6].ToUpper()}", CompanyId = r.CompanyId, Status = SNUL.Shared.Domain.Models.RFQStatus.Pending };
